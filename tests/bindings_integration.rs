    ...
    for lang in [
        ...
        match result {
            Ok(code) => {
                ...
                match lang {
                    BindingLanguage::Rust => {
                        assert!(code.contains("pub struct ContractClient"), "Missing ContractClient in Rust");
                        assert!(code.contains("impl ContractClient"), "Missing impl in Rust");
                    }
                    BindingLanguage::TypeScript => {
                        assert!(code.contains("export class ContractClient"), "Missing ContractClient in TS");
                        assert!(code.contains("export interface"), "Missing interfaces in TS");
                    }
                    BindingLanguage::Python => {
                        assert!(code.contains("class ContractClient"), "Missing ContractClient in Python");
                        assert!(code.contains("@dataclass"), "Missing dataclass in Python");
                    }
                    BindingLanguage::Go => {
                        assert!(code.contains("type ContractClient struct"), "Missing ContractClient in Go");
                        assert!(code.contains("func NewContractClient"), "Missing constructor in Go");
                    }
                }
...
fn test_error_handling() {
    ...
    let temp_file = NamedTempFile::new().unwrap();
    let result = starforge::utils::bindings::generate_bindings(temp_file.path(), BindingLanguage::Rust);
    ...
    let result = starforge::utils::bindings::generate_bindings(temp_file.path(), BindingLanguage::Rust);
    assert!(result.is_err(), "Should fail on WASM without contract metadata");
}
...
fn test_idiomatic_code_generation() {
    ...
    let languages = [
        ...
        (BindingLanguage::TypeScript, vec!["export class", "export interface", "type"]),
        (BindingLanguage::Python, vec!["class", "def", "from typing import"]),
        ...
        if let Ok(code) = result {
            for pattern in &patterns {
                assert!(code.contains(pattern), 
                       "Missing pattern '{}' in {:?} generated code", pattern, lang);
            }
        }
    }
}