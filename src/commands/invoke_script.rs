...
pub async fn handle(args: InvokeScriptArgs) -> Result<()> {
    ...
    p::header(if args.dry_run { "Invoke Script (dry run)" } else { "Invoke Script" });

    for (index, step) in script.steps.iter().enumerate() {
        let network = interpolate(step.network.as_deref().unwrap_or(&network_default), &script.env)
        ...
        config::validate_network(&network)
            .with_context(|| format!("steps[{}].network", index))?;
        let call_args = step.args.iter().map(|arg| interpolate(&arg.value, &script.env))
            .collect::<Result<Vec<_>>>()?;
        let types = step.args.iter().map(|arg| arg.arg_type.clone()).collect::<Vec<_>>();
        validate_types(&types).with_context(|| format!("steps[{}].args", index))?;
        println!("[{}] {}::{} on {}", step.name, contract_id, function, network);
        for (arg_index, (value, arg_type)) in call_args.iter().zip(types.iter()).enumerate() {
            println!("  arg[{}] ({}) = {}", arg_index, arg_type, value);
        }
        if args.dry_run { continue; }

        let wallet = cfg.wallets.iter().find(|entry| entry.name == wallet_name)
            .ok_or_else(|| anyhow::anyhow!("steps[{}].wallet: wallet '{}' not found", index, wallet_name))?;
        let outcome = soroban::invoke_contract(
            &contract_id, &function, &call_args, &types, &network, Some(wallet), None,
        ).await.with_context(|| format!("steps[{}] '{}' failed", index, step.name))?;
        let return_value = outcome.transaction.as_ref()
            .map(|tx| tx.return_value.as_str())
            .unwrap_or(&outcome.simulation.return_value);
        let assertions = step.assertions.iter().map(|assertion| {
            Ok(Assertion {
                equals: assertion.equals.as_deref().map(|value| interpolate(value, &script.env)).transpose()?,
                contains: assertion.contains.as_deref().map(|value| interpolate(value, &script.env)).transpose()?,
            })
        }).collect::<Result<Vec<_>>>()?;
...
fn load(path: &Path) -> Result<InvokeScript> {
    ...
    match path.extension().and_then(|ext| ext.to_str()).map(|ext| ext.to_ascii_lowercase()).as_deref() {
        ...
        Some("yaml") | Some("yml") => serde_yaml::from_str(&text).context("invalid invocation YAML schema"),
        _ => anyhow::bail!("invocation script must use .json, .yaml, or .yml extension"),
    }
}

fn validate(script: &InvokeScript) -> Result<()> {
    if script.version != 1 { anyhow::bail!("version: unsupported schema version {}; expected 1", script.version); }
    if script.steps.is_empty() { anyhow::bail!("steps: must contain at least one step"); }
    for (index, step) in script.steps.iter().enumerate() {
        if step.name.trim().is_empty() { anyhow::bail!("steps[{}].name: must not be empty", index); }
        if step.function.trim().is_empty() { anyhow::bail!("steps[{}].function: must not be empty", index); }
        if step.wallet.trim().is_empty() { anyhow::bail!("steps[{}].wallet: must not be empty", index); }
        ...
        for (assert_index, assertion) in step.assertions.iter().enumerate() {
            if assertion.equals.is_none() == assertion.contains.is_none() {
                anyhow::bail!("steps[{}].assertions[{}]: set exactly one of equals or contains", index, assert_index);
            }
        }
    }
    Ok(())
...
fn validate_types(types: &[String]) -> Result<()> {
    for arg_type in types {
        if !matches!(arg_type.as_str(), "string" | "symbol" | "int" | "bool" | "address") {
...
fn interpolate(value: &str, vars: &BTreeMap<String, String>) -> Result<String> {
    ...
    while let Some(start) = rest.find("${") {
        ...
        let end = rest[start + 2..].find('}')
            .ok_or_else(|| anyhow::anyhow!("unterminated environment variable in '{}'", value))? + start + 2;
        let key = &rest[start + 2..end];
        let replacement = vars.get(key).cloned().or_else(|| std::env::var(key).ok())
...
fn assert_result(index: usize, assertions: &[Assertion], value: &str) -> Result<()> {
    for (assert_index, assertion) in assertions.iter().enumerate() {
        if let Some(expected) = &assertion.equals {
            if value != expected { anyhow::bail!("steps[{}].assertions[{}]: expected return value '{}', got '{}'", index, assert_index, expected, value); }
        }
        if let Some(expected) = &assertion.contains {
            if !value.contains(expected) { anyhow::bail!("steps[{}].assertions[{}]: return value does not contain '{}'", index, assert_index, expected); }
        }
    }
    Ok(())
}
...
mod tests {
    ...
    fn parses_yaml_fixture_and_interpolates() {
        let script: InvokeScript = serde_yaml::from_str(include_str!("../../tests/fixtures/invoke_script.yaml")).unwrap();
        assert_eq!(interpolate(&script.steps[0].contract_id, &script.env).unwrap(), "C...");
        validate(&script).unwrap();
    }

    #[test]
    fn rejects_unknown_fields_and_bad_assertions() {
        assert!(serde_json::from_str::<InvokeScript>(r#"{"version":1,"steps":[],"extra":true}"#).is_err());
        let script: InvokeScript = serde_json::from_str(r#"{"version":1,"steps":[{"name":"x","contract_id":"C...","function":"f","wallet":"w","assertions":[{}]}]}"#).unwrap();
        assert!(validate(&script).unwrap_err().to_string().contains("exactly one"));
    }

    #[test]
    fn assertions_cover_equals_and_contains() {
        let assertions = vec![
            Assertion { equals: Some("ok".into()), contains: None },
            Assertion { equals: None, contains: Some("o".into()) },
        ];
